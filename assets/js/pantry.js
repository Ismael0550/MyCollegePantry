...
form.onsubmit = async (e) => {
  ...
  const expStr = document.getElementById('f_expiresOn').value.trim();
  const data = {
...
function render() {
  ...
  for (const it of items) {
    ...
    const expDate = it.expiresOn ? parseDateLocal(it.expiresOn) : null;

    const isRed = isFinite(dLeft) && dLeft <= 3;
    const isYellow = isFinite(dLeft) && dLeft >= 4 && dLeft <= 9;
    const isGreen = isFinite(dLeft) && dLeft > 9;

   let statusClass = '';
   if (isRed)      statusClass = 'expiring';
   else if (isYellow) statusClass = 'low';
   else if (isGreen)  statusClass = 'fresh';

// qty-low only if no expiry class chosen
const qtyLow = !statusClass && ((it.qty || 0) <= 1);
...
const flags = [
  ...
  qtyLow ? 'low' : ''
    ...
    el.innerHTML = `
          ...
          <div class="row">
            <span class="muted">${expDate ? 'Expires ' + expDate.toLocaleDateString() : 'No date'}</span>
...
// Replace UTC logic add Local date
